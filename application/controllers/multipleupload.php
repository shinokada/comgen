...
class Multipleupload extends CI_Controller {
    ...
    private $version;
    private $appname;
    private $dbfile;   // path to data file
    private $expire; 
    ...
    function __construct()
    {
            ...
            $this->dbfile= "counter/visitors.txt";
            $this->expire= 60; 
            // end of visitor counter para
            */
    ...
    function index()
    {
       // $data['error']="";
        ...
        //$data['visitorscounter']=CountVisitors($this->dbfile, $this->expire);
 ...
 */       
    function do_upload()
    {
        ...
        if($this->input->post('allinone')){
            $data['allinone']=$this->input->post('allinone');
        }
        if($this->input->post('mycommentarea')){
            $data['mycommentarea']=$this->input->post('mycommentarea');
        }
        ...
        if ( ! $this->upload->do_upload('userfile'))
        {
            ...
            $this->load->view('multiple/container', $data);
        }  elseif (!$this->input->post('showibgrade') AND !$this->input->post('lacomment') AND 
            !$this->input->post('lagocomment') AND !$this->input->post('mathgocomment') AND 
            !$_FILES['userfile1']['name'] AND !$this->input->post('mycommentarea')) 
        {
...
// $data=explode("\n", $filestring);//change to array from string
    ...
    function create(){
        ...
        if ($this->input->post('showibgrade')==="1" OR $this->input->post('lacomment')==="1" OR $this->input->post('lagocomment')==="1" 
                OR $this->input->post('mathgocomment')==="1" OR $this->input->post('mycomment')==="1" OR $this->input->post('mycommentarea')==="1")
        {
            ...
            $data['title']=$this->appname;
            if($this->input->post('allinone')==="1")
                {
                // if allinone mode is on then use other view
                    $data['page']="multiple/outputallinone";
                }
                else
                {
